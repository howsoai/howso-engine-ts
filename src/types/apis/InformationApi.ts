...
import type { PlatformVersion, TraineeInformation } from "../models";
import { PlatformVersionFromJSON, TraineeInformationFromJSON } from "../models";
...
export class InformationApi extends runtime.BaseAPI {
   ...
   * Trainee information
   */
  async getTraineeInformationRaw(
    requestParameters: GetTraineeInformationRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<TraineeInformation>> {
    if (requestParameters.trainee_id === null || requestParameters.trainee_id === undefined) {
      throw new runtime.RequiredError(
        "trainee_id",
        "Required parameter requestParameters.trainee_id was null or undefined when calling getTraineeInformation.",
      );
    }
    ...
    const response = await this.request(
      {
        path: `/v2/trainee/{trainee_id}/info`.replace(
          `{${"trainee_id"}}`,
   ...
   * Trainee information
   */
  async getTraineeInformation(
    trainee_id: string,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<TraineeInformation> {
  ...
  async getVersionRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<PlatformVersion>> {
    const queryParameters: any = {};
